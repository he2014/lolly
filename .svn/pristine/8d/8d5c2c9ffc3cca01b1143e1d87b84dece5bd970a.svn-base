...
<section>
  ...
  <el-row :style="{width:'40%'}">
    <el-col>
      <el-form v-loading="listLoading" size="small" :model="data" status-icon ref="ruleForm2" label-width="100px" class="demo-ruleForm">
        <el-form-item label="渠道" prop="paly">
          <el-select v-model="data.paly" placeholder="请选择">
            <el-option v-for="item in palyoptions" :key="item" :label="item" :value="item">
            </el-option>
          </el-select>
        </el-form-item>
  ...
  <el-row :style="{paddingTop:'30px',paddingLeft:'100px'}">
    <el-button size="mini" type="primary" @click="dialogVisibl(true)">提交</el-button>
    <el-button size="mini" @click="dialogVisibl(false)">重置</el-button>
  </el-row>
</section>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    returnUserMan() {
      this.$router.push('/recharge/android')
    }
  }
}
</script>
